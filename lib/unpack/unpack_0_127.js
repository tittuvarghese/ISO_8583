...
module.exports = function (incoming, isoJSON, config) {
  if (Buffer.isBuffer(incoming)) {
    ...
    if (!secondaryBitmap && config.bitmapEncoding === 'utf8') bitmapEnd = 20;
    else if (secondaryBitmap && config.bitmapEncoding === 'utf8') bitmapEnd = 36;

    if (!secondaryBitmap && config.bitmapEncoding !== 'utf8') bitmapEnd = 12;
    else if (secondaryBitmap && config.bitmapEncoding !== 'utf8') bitmapEnd = 20;
