...
class Main {
  ...
  setBitMapEncoding(encoding) {
    return this.bitmapEncoding = encoding;
  }

  setSecondaryBitmap(status) {
    status ? this.secondaryBitmap = true : this.secondaryBitmap = false;
  }
  ...
  getBitMapHex() {
    ...
    if (state.error) {
      ...
      if (this.bitmaps !== null && msgTypes(this.MsgType)) {
        ...
        return 16, maps.join('');
      } else return T.toErrorObject('bitmap error, expecting 128 or 64 length unit array');
    }
  }
  ...
  hasSecondaryBitmap(primaryBitmapBuffer, config) {
    if (config.secondaryBitmap === false) return false;
  ...
  buildBitmapBuffer(bitmap, type) {
    ...
    if (this.secondaryBitmap && (this.bitmapEncoding === 'utf8' || type === 'ascii')) {
      return Buffer.alloc(32, bitmap.toUpperCase());
    } else if (!this.secondaryBitmap && (this.bitmapEncoding === 'utf8' || type === 'ascii')) {
      return Buffer.alloc(16, bitmap.toUpperCase());
    } else if (!this.secondaryBitmap) return Buffer.alloc(8, bitmap, 'hex');
